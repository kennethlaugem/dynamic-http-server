...
function HttpServer(options) {
  ...
  before.push(function(req, res) {
    let root = './'
    let fpath = path.join(root, req.url)

    if (fs.existsSync(fpath)) {
      let extension = path.extname(fpath)

      if (extension == '.html') {
        let regex = new RegExp('{{include (.*)}}', 'gi')
        let file = fs.readFileSync(fpath, { encoding: 'utf8' })
        ...
        if (matches.length) {
          matches.forEach(function(item, index) {
            if (fs.existsSync(item)) {
              let includeFile = fs.readFileSync(item, { encoding: 'utf8' })
              let placeholder = '{{include ' + item + '}}'
              file = file.replace(placeholder, includeFile)
            }
          })
          return res.end(file)
        }
